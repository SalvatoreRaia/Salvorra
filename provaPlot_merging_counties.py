...
#IMPORTING DATA
filename = r"C:\\Users\Salvo\Desktop\IFISC\data\\r_f_county_w\\freq_ranking_"+county+'(' + str(c) +')_'+str(41507)+"w.csv"
dat = pd.read_csv(filename)
print(dat)

parole = dat.iloc[:-16, 1]
print(parole)
parole = parole.values.tolist()
print(parole)
word1 = 13
...
which_county = [3074]#1,141, 12, 200, 134, 103, 34, 84, 100, 180]
...
for i in which_county:
    ...
    ax1.scatter(np.arange(1,len(y[i-1][:word1])+1),y[i-1][:word1],
             s = 10, label='First 14 words in Brown (Illinois)', color='xkcd:royal blue')
    #'PMF_'+str(i)+' '+ Adata[1][i] +', '+str(b)+' words.'
...
'''
#Words annotations
x = np.arange(1,int(word1)+26+1)
print(y[3073])
for i, parole in enumerate(parole):
    print(i)
    ax1.annotate(str(parole), (x[i] - 0.0006, y[3073][i] + 0.0006))
    print(x[i], y[3073][i])
    print('SUCA',i)
'''
# Figure settings
...
print('SUCAAAAAAAAAAAAAAAAAA')
print(myD)
print(len(myD), len(myD[0]), len(myD[1]), len(myD[3074]))
...
print('SUCAAAAAAAAAAAAAAAAAA')
print(js_D)
print(len(js_D), len(js_D[0]), len(js_D[1]), len(js_D[3074]))
