...
frames = 1800
o_p = 22
time_span=np.linspace(0,o_p,frames) #35 orbital periods and 3500 points 
#the ratio between the two set the velocity of the simulation (0,20,500) #20 orbital periods and 500 points
...
ax.set_zlim(zmin-0.01*(zmax-zmin),zmax+0.01*(zmax-zmin))

"""
x_points = [ 1000, -1000, 0, 0, 0, 0]
y_points = [ 0, 0, 1000, -1000, 0, 0]
z_points = [ 0, 0, 0, 0, 1000, -1000]
...
#I want a n second track
n_track = 0.5 #0.5 second

duration = 30 #seconds of matplotlib animation
track = int(n_track * frames/duration)
...
def animate(i, data, lines, pts):
    ...
    for line, pt, data in zip(lines, pts, data):
        ...
        if i > track :
            line.set_data(data[0:2, i-track:i])
            line.set_3d_properties(data[2, i-track:i])
        else :
            line.set_data(data[0:2, 1:i])
            line.set_3d_properties(data[2, 1:i])
...
ani = animation.FuncAnimation(fig, animate, frames= frames,
                               blit=True, fargs = (data, lines, pts), repeat=True, interval = duration *1000 /frames)#, init_func=init)

#For the display animation the duration is going to be frames * interval / 1000 (in seconds),  duration * o_p *1000 /frames = interval 
#For the saved animation the duration is going to be frames * (1 / fps) (in seconds)  frames = duration *fps

#To use the line below you need to install imagemagick


ani.save('C:\\Users\\Salvo\\Desktop\\tribodimp4.mp4', writer='imagemagick', fps=60)
plt.show()
